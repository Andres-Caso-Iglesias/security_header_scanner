  ...
  generatePdf(result: ScanResult): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // ================================================================
        // Color palette - sober, high contrast, print-friendly
        // ================================================================
        ...
        function horizontalLine(y: number, width?: number): void {
        ...
        if (result.tls && result.tls.checked) {
          ...
          if (result.tls.error) {
            ...
            if (y > doc.page.height - certHeight - 50) { doc.addPage(); y = 50; }
        ...
        for (const h of result.headers) {
          if (y > doc.page.height - 85) { doc.addPage(); y = 50; }
        ...
        doc.fontSize(7).fillColor(lightGray).text(
          `Generado el ${new Date().toLocaleString('es-ES')} por Auditoria Web Scanner v1.0`,
          50, y + 5, { width: pageWidth, align: 'center' }
        );
